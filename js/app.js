...
window.TF={nav:nav,subNav:subNav,load:loadData,start:tmrStart,pause:tmrPause,done:tmrDone,addTimeToTask:addTimeToTask,addTask:addTask,quickAdd:quickAdd,openAddModal:openAddModal,
  ...
  rvToggle:rvToggle,rvToggleAll:rvToggleAll,mergeReviewSelected:mergeReviewSelected,dismissReviewSelected:dismissReviewSelected,backfillTasks:backfillTasks,setRvFilter:setRvFilter,setRvSort:setRvSort,
  filt:function(k,v){S.filters[k]=v;render()},filtSearch:function(v){S.filters.search=v;clearTimeout(S._searchTmr);S._searchTmr=setTimeout(function(){render();var si=document.querySelector('.fl-s');if(si){si.focus();si.selectionStart=si.selectionEnd=si.value.length}},250)},clearF:function(){S.filters={followUp:'',client:'',endClient:'',campaign:'',project:'',opportunity:'',cat:'',imp:'',type:'',search:'',dateFrom:'',dateTo:''};render()},
  filtNav:function(k,v){S.filters={followUp:'',client:'',endClient:'',campaign:'',project:'',opportunity:'',cat:'',imp:'',type:'',search:'',dateFrom:'',dateTo:''};S.filters[k]=v;nav('tasks')},
  ...
  openOpportunityDetail:openOpportunityDetail,saveOpportunity:saveOpportunity,oppAutoSave:oppAutoSave,addOppUpdate:addOppUpdate,toggleFeeType:toggleFeeType,createOppFollowUp:createOppFollowUp,
  ...
  openIntegrationsModal:openIntegrationsModal,triggerSync:triggerSync,cleanupDuplicates:cleanupDuplicates,saveIntegrationBtn:saveIntegrationBtn,testIntegrationBtn:testIntegrationBtn,deleteIntegrationBtn:deleteIntegrationBtn,connectGmail:connectGmail,connectReadai:connectReadai,setFinDirection:setFinDirection,setUpcomingDate:setUpcomingDate,
  /* Scheduled Items */
  ...
  loadAiDrafts:loadAiDrafts,getAiDraftCount:getAiDraftCount,triggerBatchDraft:triggerBatchDraft,
  ...
  openSignatureEditor:openSignatureEditor,sigInsertLink:sigInsertLink,toggleComposeSig:toggleComposeSig,renderRecipientChips:renderRecipientChips,handleComposeFiles:handleComposeFiles,renderComposeAttachments:renderComposeAttachments,
  ...
  toggleCampaignStatus:toggleCampaignStatus,setInstantlyCampaignTab:setInstantlyCampaignTab,
  ...
  accountAction:accountAction,toggleAccountStatus:toggleAccountStatus,toggleAccountWarmup:toggleAccountWarmup,
  openAccountDetail:openAccountDetail,openInstantlyAccountDetail:openInstantlyAccountDetail,
  refreshActivity:refreshActivity,fetchGA4Realtime:fetchGA4Realtime,startGA4Polling:startGA4Polling,stopGA4Polling:stopGA4Polling,toggleGA4Config:toggleGA4Config,addGA4LabelRow:addGA4LabelRow,saveGA4Labels:saveGA4Labels,
  /* Meetings */
