...
var ICONS={
  ...
  chevron_left:'<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',
...
var OPP_TYPES={
  ...
  fc_partnership:{label:'F&C Partnerships',short:'F&C-P',
    stages:['Brief Received','Meeting Booked','Meeting Complete','Video Tracking','Pitch Development','Proposal Delivered','Renewal'],
  ...
  fc_direct:{label:'F&C Direct',short:'F&C-D',
    stages:['Lead Generated','Discovery Call','Video Tracking','Proactive Pitch','Negotiation','Renewal'],
    closedStages:['Closed Won','Closed Lost'],
...
function restore(){try{var t=localStorage.getItem('tf_t');if(t)S.timers=JSON.parse(t);
  ...
  var opcw=localStorage.getItem('tf_opcw');if(opcw)S.opShowClosedWon=true;
  var opcl=localStorage.getItem('tf_opcl');if(opcl)S.opShowClosedLost=true;
  var opcf=localStorage.getItem('tf_opcf');if(opcf)S.opClientFilter=opcf;
...
async function cacheUserEmail(){
  ...
  try{
    ...
    /* Get actual Gmail address from Gmail Profile API (reliable, not dependent on thread data) */
    var resp=await fetch('/api/gmail/profile',{headers:{'Authorization':'Bearer '+token}});
    if(resp.ok){
      var data=await resp.json();
      var gmailAddr=(data.email||'').toLowerCase().trim();
...
async function loadData(){toast('Loading data...','info');
  try{
    ...
    await Promise.all([loadPayments(),loadCampaignMeetings(),loadOpportunityMeetings(),loadActivityLogs(),loadFinancePayments(),loadPayerMap(),loadIntegrations(),loadAccountBalances(),loadScheduledItems(),loadCampaignNotes(),loadClientNotes(),loadGmailThreads(),loadContacts(),cacheUserEmail(),loadMeetings(),loadInvoicesSafe()]);
...
async function backfillEndClientIds(){
  /* Backfill end_client_id from end_client name match for CRM-facing tables only.
     knowledge_chunks is intentionally excluded — its rows are regenerated by the
     entity-sync pipeline and direct updates were returning 500s on large rowsets. */
  var allTables=['tasks','done','review','campaigns','opportunities','contacts','finance_payments','finance_payment_splits','meetings','gmail_threads'];
  var tables=[];
