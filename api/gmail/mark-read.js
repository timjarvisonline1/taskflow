 ...
 * Marks a Gmail thread as read or unread.
 * Body: { threadId, unread: true/false }
 */
module.exports = async function handler(req, res) {
  ...
  const { threadId, unread } = req.body || {};
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error('Gmail mark-read failed (' + resp.status + '): ' + err.substring(0, 200));
    }
