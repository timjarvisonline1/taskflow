...
const { exchangeReadaiCode } = require('../_lib/readai-auth');

/**
 * GET /api/auth/readai-callback
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!credRow) {
      return res.status(400).send(errorPage('Read.ai credentials not found. Save your Client ID and Secret first.'));
    }
    ...
    const tokens = await exchangeReadaiCode(creds.client_id, creds.client_secret, code, redirectUri);
...
function successPage() {
  return `<!DOCTYPE html><html><head><title>Read.ai Connected</title>
...
<body><div class="box"><h1>Read.ai Connected</h1><p>You can close this tab and return to TaskFlow.</p>
<p style="margin-top:16px;font-size:12px;color:#6b7280">This window will close automatically...</p></div>
<script>if(window.opener){window.opener.postMessage('readai-connected','*')}setTimeout(function(){window.close()},2000)</script></body></html>`;
}

function errorPage(msg) {
  return `<!DOCTYPE html><html><head><title>Read.ai Connection Failed</title>
<style>body{font-family:-apple-system,system-ui,sans-serif;background:#0d0d0f;color:#e5e7eb;display:flex;align-items:center;justify-content:center;height:100vh;margin:0}
...
<body><div class="box"><h1>Connection Failed</h1><p>Something went wrong connecting Read.ai.</p>
